...
컴포지션은 자동차의 트랜스미션빼면 다른 트랜스미션넣으면 되고~



격리, 모듈화가 잘되어있는거 - 여기서 분해는 `composition` 을 의미.
...
인지과부화, 복잡성 폭발 / 어느수준이 넘어가면 이해할 수 없다.



if가 2단계 넘어가면 복잡성 폭발이라고 생각할 수있다.
...
사람의 뇌가 flow처리를 선호하기 때문에 어떤 문제를 해결하기 위해서 flow로 처리하려는 경향이 있다.



마치 오늘의 Todo를 정하고 해치는 듯한 것.



flow chart 기법의 단점은, 사람은 학습한다는 점이다. 어떤 주황색 flow를 처리하는데,
...
전체가 오염되는 상황이 발생해서 우리가 코딩할 때 밤을 새는 이유이다.



이런 식의 코드는 딱 필요한 부분만 짜기 때문에 compact하게 코드를 딱딱 맞게 쓸 수 있지만, 이건 잘 못된 것이다.
...
위 데이터 타입은 데이터를 기준으로 세상을 바라보기 시작한다.

그래서, 리스코프 치환원칙을 이야기하기 전에는 객체지향원칙이 발달하지 않았다.



데이터에 따라서 함수를 말들어봐-

해야될 일을 데이터에 맞쳐서 함수를 만든다면, 아래와 같을거야
...
하는일이 비슷한 함수들이 있다는 것을 발견하게 될거야. 

정규직, 시간제 ~ tax를 계산하는 것은 똑같아. 해야될 일이 비슷



이러한 시각으로 바라보야 한다는 점이다. Flow 안에서 공통점을 찾을 수 있느냐 없느냐-

추상화된 레벨에서 바라봐야 한다. (어떤 데이터이든지 나는 `이걸` 해줄 것이다 라는 것)
...
![image-20190925004431842](https://tva1.sinaimg.cn/large/006y8mN6gy1g7b1ee5wnej30ly0jewfx.jpg)
 ...
 코드의 응집도가 높아진다. 물론, 단점은 케이스가 늘어날때마다 하나씩 커지지만- `지식을 한 곳에 모을 수 있다`라는 장점이 있다.

외부에 이걸 하나의 형으로 노출하고 싶다. 라면, `하늘색 주황색 빨간색 을 하나라고 퉁칠 수 있다.`



고도로 추상화를 쓰고 있는 것처럼 보이나, 단점은,

아 장점은 외부에 노출시키지않고 내부에 모든 문제를 해결할 것이다.

![image-20190925004737362](https://tva1.sinaimg.cn/large/006y8mN6gy1g7b1hm2bdyj30o00li0uf.jpg)



ADT가 좋다라는 논리를 깨기위해서는 객체지향 프로그래밍을 잘알아야한다. 
...
기능이 추가되지 않는 선에서 ADT가 좋을 수 있다. 그러나 상태가 추가되거나 변형이 일어날 때는 
...
압도적으로 유리한데, 초록색이 추가될 때 상태추가가 유리.
...
여기서 성급한 추상화가 가장 위험하다. 피해야 한다. 추상화를 계속 정교화해서 Key case를 만들어야 한다.
...
모든 소프트웨어는 변한다. 라는 원칙에 의하면 ADT는 취약하다.
...
이름을 길게쓸 수밖에 없다. 한 곳에 모든 기능을 모았기 때문.

변수명부터 벌써 충돌이 발생한다. 통합하면서 코드가 변화된다.



클래스 2개 이상을 하나로 통합했음. 잘된것처럼 보임. 모든 지식이 한 곳에 모임.
